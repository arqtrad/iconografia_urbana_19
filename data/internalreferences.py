...
class ReferenceManager(object):
    ...
    def __init__(self, autoref=True, numbersections=True):
        if autoref:
            self.replacements = {'figure': 'Figura {}',
                                 'section': 'Seção {}',
                                 'table': 'Tabela {}',
                                 'math': 'Fórmula {}'}

            self.multi_replacements = {'figure': 'Figuras ',
                                       'section': 'Seções ',
                                       'table': 'Tabelas ',
                                       'math': 'Fórmulas '}
        elif not autoref:
    ...
    def figure_replacement(self, key, value, format, metadata):
        ...
        if 'unnumbered' in attr[1]:
            ...
            if caption:
                ...
                fcaption = [pf.Str('Figura'), pf.Space(), pf.Str(str(ref['id']))]
    ...
    def tableattrs_replacement(self, key, value, format, metadata):
        ...
        if 'unnumbered' in classes:
            ...
            if caption:
                ...
                fcaption = [pf.Str('Tabela'), pf.Space(), pf.Str(str(ref['id']))]
